...
if (!empty($arrNav[substr(@$category_code,0,2)]['sub'])) { // 2depth 가 있을경우 표출
    echo '
    <div class="cateBox" id="cateBoxDepth2">
		<div id="btnCateBox"></div>
        <ul class="category_list--fc depth2">';
    $index = 1;
    foreach($arrNav[substr(@$category_code,0,2)]['sub'] as $key => $val) {
        ...
        if (!empty($val['title']) and $val['status'] == 'y') {
            if ((empty($arrUrl[1]) and $index == 1) or @$arrUrl[1] == $val['url']) {
                ...
                if($val['is_outer_link'] == 'n') echo '<li><a data-target="'.$val['url'].'Target" href="'.$prefix_link.'/'.$val['full_url'].'">'.$val['title'].'</a></li>';
