	...
	commonEffect: function(){
		...
		$('.common_effect').each(function(){
			...
			var tween1 = function(){
				...
				var el_right = _.find('.ce_item_right');
				$.each(el, function(e){
					var _ = $(this);
					setTimeout(function() {
				...
				$.each(el_right, function(e){
					var _ = $(this);
					setTimeout(function() {
						TweenLite.fromTo(_, 0.5, {position: 'relative', autoAlpha:0, right: '50px'}, { autoAlpha:1, right: 0} )
					}, e * 400);
				})
			}
		...
		$('.common_effect__ul').each(function(){
			var _ = $(this);
			var tween = function(){
				var el_up = _.find('.up_item');
				var el_down = _.find('.down_item');
				var el = _.find('>li');
                
				$.each(el_up, function(e){
					var _ = $(this);
					setTimeout(function() {
						TweenLite.fromTo(_, 0.5, {position: 'relative', autoAlpha:0, bottom: '-30px'}, { autoAlpha:1, bottom: 0} )
					}, e * 300);
				})
                $.each(el_down, function(e){
					var _ = $(this);
					setTimeout(function() {
						TweenLite.fromTo(_, 0.5, {position: 'relative', autoAlpha:0, bottom: '30px'}, { autoAlpha:1, bottom: 0} )
					}, e * 300);
...
$(function(){
	if( $('.img-bg-copy').length != 0 ) {
		$('.img-bg-copy').each(function(){
			var imgSrc = $(this).children('img.bg').attr('src');
